...
const Exam = () => {
  ...
  const sessionType = isStandalone
    ? AssessmentSessionType.STANDALONE_EXAM
    : AssessmentSessionType.COURSE_EXAM;
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        await prepareForExam(sessionType, parseInt(examId, 10));

        sessionKeyRef.current = buildSessionKey(sessionType, parseInt(examId, 10));
        ...
        if (!getSession(sessionKeyRef.current)) {
          startSession({
            type: sessionType,
            targetId: parseInt(examId, 10),
            questionIds,
            durationSeconds: (data.duration || 30) * 60,
            title: data.title,
          });
        }
      } catch (err) {
    ...
    fetchExam();
  }, [examId, isStandalone, navigate, sessionType]);
  ...
  if (loading) {
    return (
      <div className="centered" style={{ height: '100vh', background: '#020617', color: 'white' }}>
        Encrypting Exam Paper...
      </div>
    );
  }
    ...
    <div style={{ background: '#020617', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <div className="flex" style={{ gap: '1rem' }}>
          <div style={{ padding: '0.5rem', background: 'rgba(79, 70, 229, 0.1)', borderRadius: '0.5rem' }}>
            <HelpCircle color="var(--primary)" />
          </div>
          <div>
            <p style={{ color: 'var(--text-sub)', fontSize: '0.7rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>
              Secure Assessment
            </p>
        ...
        <div
          style={{
            ...
            background: timeLeft < 300 ? 'rgba(239, 68, 68, 0.1)' : 'rgba(15, 23, 42, 0.5)',
            borderRadius: '3rem',
            border: `1px solid ${timeLeft < 300 ? '#ef4444' : 'var(--border)'}`,
            color: timeLeft < 300 ? '#f87171' : 'white',
            fontWeight: 800,
            fontSize: '1.4rem',
            display: 'flex',
            gap: '0.75rem',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div className="glass-card fade-in" style={{ padding: '4rem', background: 'rgba(30, 41, 59, 0.3)' }}>
            <div className="space-between" style={{ marginBottom: '3rem' }}>
              ...
              >
                Complexity: {currentQuestion?.degree || 1} pts
              </span>
            </div>

            <h3 style={{ fontSize: '1.8rem', marginBottom: '3.5rem', lineHeight: '1.5', fontWeight: 500 }}>
          ...
          <div className="glass-card" style={{ padding: '2.5rem', position: 'sticky', top: '100px' }}>
            ...
            <div style={{ marginTop: '2.5rem', paddingTop: '2rem', borderTop: '1px solid var(--border)' }}>
              <div className="flex" style={{ gap: '1rem', color: 'var(--text-sub)', fontSize: '0.85rem' }}>
                ...
                <p>إذا خرجت من الامتحان تُحفظ إجاباتك وتُرسل تلقائياً، ويمكنك الدخول مرة أخرى.</p>
              </div>
            </div>
          </div>
        </div>
